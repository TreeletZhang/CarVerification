...
if __name__ == '__main__':
    if args.train:
        ...
        env = UnityWrapper(train_mode=False, base_port=5004) #RollerBall
    ...
    if args.train:
        ...
        for episode in range(TRAIN_EPISODES):
            ...
            for step in range(MAX_STEPS):  # in one episode
                ...
                action = agent.get_action(state)
                # print("action:", action)
                # print("type_action:", type(action))
                action_to_unity = action[np.newaxis, :]
                # print("action_to_unity:", action_to_unity)
                # print("shape_action:", action_to_unity.shape)
                obs_list, reward, done, max_step = env.step(None, action_to_unity)
...
# if __name__ == '__main__':
#     env = UnityWrapper(train_mode=True, base_port=5004)  # CarVerification
#     obs_shape_list, d_action_dim, c_action_dim = env.init()
#     print("obs_shape_list", obs_shape_list)
#     print("d_action_dim:", d_action_dim)
#     print("c_action_dim:", c_action_dim)
