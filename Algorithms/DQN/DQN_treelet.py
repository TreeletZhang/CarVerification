...
def get_model(inputs_shape):
    ...
    visual_embedding1 = tl.layers.Conv2dLayer(shape=(3, 3, 3, 32), strides=(1, 1, 1, 1), act=tf.nn.relu,
                                              name='conv2d_1')(ni)
    visual_embedding2 = tl.layers.Conv2dLayer(shape=(5, 5, 32, 64), strides=(1, 1, 1, 1), act=tf.nn.relu,
                                              name='conv2d_2')(visual_embedding1)
    # shape设置卷积核的形状，前两维是filter的大小，第三维表示前一层的通道数，也即每个filter的通道数，第四维表示filter的数量
    inputs = tl.layers.Flatten()(visual_embedding2)
    nn = tl.layers.Dense(4, act=None, W_init=tf.random_uniform_initializer(0, 0.01), b_init=None, name='q_a_s')(inputs)
...
if __name__ == '__main__':
    env = UnityWrapper(train_mode=True, base_port=5004) #RollerBall
    ...
    qnetwork = get_model([None, 64, 48, 3])
    qnetwork.train()
